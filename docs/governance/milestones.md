...
| M4 - certified tessellation floor | IN_PROGRESS | Every valid or conservatively repairable face produces a deterministic boundary-exact intersection-free triangle mesh | The exact-predicate reference CDT now supports direct planar holes through exact visibility bridges and feeds a no-weld global `CertifiedMesh`. The closed planar box proves 20 canonical vertices, 36 oriented triangles, 54 two-sided/oppositely wound edges, complete provenance/surface checks, and order-independent fingerprinting; a real through-hole face also certifies as an explicitly open face product. Degenerate/general cut-graph completeness, curved surfaces, adaptive error bounds, critical regions, 3D triangle intersections, production routing, Linux determinism, and corpus completion remain open. |
| M5 - non-vacuous validation | IN_PROGRESS | All required validator coverage is non-zero/complete; twisted-wall and vacuous-provenance witnesses fail before their fixes | Trim, CDT, and certified planar-body certificates report expected/checked/skipped/failed and refuse vacuous face coverage. Remaining independent source-incidence/Euler, curved face bounds, triangle/triangle, twisted-wall, and deliberately vacuous provenance gates remain open. |
