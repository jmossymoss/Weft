...
| M3 - predicates, counts, canonical boundaries | IN_PROGRESS | Every valid shared edge has one bijective sample sequence and consistent coedge UV uses | Exact count assignment, canonical samples, bounded UV uses/lifts, azimuth registration, exact dyadic predicates, and non-vacuous planar trim-domain validation pass strict/static adversarial batteries; see the four M3 evidence records. Face/coedge loop assembly, critical segmentation, sum constraints, exact-construction CDT, degenerate singularities, full periodic closure proof, and Linux determinism remain open. |
