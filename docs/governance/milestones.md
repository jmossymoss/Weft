...
| M3 - predicates, counts, canonical boundaries | IN_PROGRESS | Every valid shared edge has one bijective sample sequence and consistent coedge UV uses | Exact count assignment, atomic shared-edge sample IDs/indices, bounded stored/planar UV uses, periodic lifting, and azimuth registration pass strict/static box, cylinder, and partial-arc witnesses; see `docs/evidence/m3-interval-solver-2026-07-17.md` and `docs/evidence/m3-canonical-boundaries-2026-07-17.md`. Critical segmentation, sum constraints, exact predicates/CDT, degenerate singularities, and full periodic closure proof remain open. |
