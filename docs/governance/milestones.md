...
|---|---|---|---|
| M0 - baseline and governance | IN_PROGRESS | Legacy baseline is reproducible, production routing has one owner, and governance/build routes are explicit | MSVC strict and static-analysis presets build the complete product. Strict CTest reproduces exactly the eight baseline failures; process-isolated secure tests also pass concurrently across both MSVC lanes. The primary `mesh` CLI and desktop app now have only the secure generator; B-rep-to-mesh `convert`, `sweep`, `cache-check`, dormant source removal, and Linux execution remain open. See the M0/M7 evidence records. |
...
| M7 - workflow restoration | IN_PROGRESS | CLI, recipe migration, app, editing, exports, hot reload, and Blender link use `MeshingResult` with no legacy selector | `weft mesh`, app async regeneration, export finalization, audited hot reload, and Blender live link consume certified results with no legacy selector. Safe recipe v1 globals migrate; per-entity/manual conflicts and recipe saving are blocked visibly. Recipe v2, editing, overlays, secure caching/proxy updates, `convert`, `sweep`, `cache-check`, and LOD report naming remain open. |
