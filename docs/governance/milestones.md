...
|---|---|---|---|
| M0 - baseline and governance | IN_PROGRESS | Legacy baseline is reproducible, production routing has one owner, and governance/build routes are explicit | MSVC strict and static-analysis presets build the complete product. Strict CTest reproduces exactly the eight baseline failures; process-isolated secure tests also pass concurrently across both MSVC lanes. The primary `mesh` CLI route now has only the secure generator; app, `convert`, and `cache-check` legacy routing plus Linux execution remain open. See the M0/M7 evidence records. |
...
| M6 - planar and cylinder templates | IN_PROGRESS | Box, full/partial cylinder, mismatched-frame cylinder, and connected plane/cylinder fixtures pass end to end | Secure core orchestration and the primary CLI export route now pass the box, full capped cylinder, and connected through-hole fixtures with deterministic certified output. Modelling still aliases the safe floor. Partial cylinders, interior axial rings, independently certified modelling polygons/quads, mismatched-reference fixtures, and app workflow routing remain open. |
| M7 - workflow restoration | IN_PROGRESS | CLI, recipe migration, app, editing, exports, hot reload, and Blender link use `MeshingResult` with no legacy selector | `weft mesh` now consumes only `MeshingResult`, preserves certified indices/UV anchors through `PolyMesh`, exports OBJ/GLB/FBX, writes complete reports, ignores old pipeline selection visibly, and blocks unmigrated recipe/manual/per-entity controls. App, Blender, recipe v2 migration, editing, hot reload, `convert`, `cache-check`, and LOD report naming remain open. |
